...
# This creates a view to display a recommendation list for a signed-in user.
@login_required
def user_recommendation_list(request):
	# get user reviews
	user_reviews = Review.objects.filter(user_name=request.user.username).prefetch_related('wine')
	# from the reviews, get a set of wine IDs so we can see what wasn't reviewed
	...
	#it's very simple and straightforward, but not really machine learning /recommender
	return render(
		request, 'reviews/user_recommendation_list.html', 
