...
"""
The update_clusters function performs cluster assignment in three steps. 
It only updates the assignment if the total number of reviews satisfies
our requirements 

So
1) Create Sparse Matrix using user ratings
2) Perform K Means - dok -> csr in order to allow for better calculations
3) Update Cluster Assignments
    - Delete previosu clusters, create new ones, for each result we add
    a user the cluster that it belongs.
"""
def update_clusters():
